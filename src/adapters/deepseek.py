...
Supports DeepSeek-V3, DeepSeek-Coder, and DeepSeek-R1 (reasoning).
"""
...
class DeepSeekAdapter(BaseAdapter):
    ...
    Known for: Excellent code generation, very cost-effective,
    strong reasoning capabilities (R1).
    """
    
    DEFAULT_MODEL = "deepseek-chat"
    
    def __init__(self, api_key: str, api_base: str = "https://api.deepseek.com", **kwargs):
        super().__init__("deepseek", api_key, api_base, **kwargs)
        self.client = httpx.AsyncClient(
            ...
            timeout=120.0,  # DeepSeek can be slower
        )
    
    async def authenticate(self) -> bool:
        """Check API key by fetching balance."""
        try:
    ...
    def _parse_response(self, data: Dict, latency_ms: int) -> AdapterResponse:
        """Parse DeepSeek response to standardized format."""
        ...
        # Include reasoning content if present (R1 model)
