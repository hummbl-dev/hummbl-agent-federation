...
if [[ -z "$MODEL" || -z "$INPUT" ]]; then
  echo "Usage: run-anthropic-governed.sh --model <model> --input <text> [--max-tokens N] [--system <text>] [--date YYYY-MM-DD] [--name prefix]" >&2
  exit 1
fi
...
"${ROOT_DIR}/scripts/run-cmd.sh" \
  ...
  --date "${DATE_STR}" \
  --name "${NAME}" \
  -- node "${ROOT_DIR}/packages/runners/claude-code/scripts/anthropic-message.cjs" \
    --request "${REQ_PATH}" \
    --response "${RES_PATH}" \
    --meta "${META_PATH}"
...
LOG_SCRIPT="${ROOT_DIR}/packages/runners/claude-code/scripts/log-run.sh"
