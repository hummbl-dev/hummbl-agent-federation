...
if (!requestPath || !responsePath || !metaPath) {
  console.error("Usage: openai-response.cjs --request <file> --response <file> --meta <file>");
  process.exit(1);
}
...
let networkPolicy = { allowlist: { domains: ["api.openai.com"] }, default: {} };
try {
  ...
  (res) => {
    ...
    res.on("end", () => {
      ...
      if (status < 200 || status >= 300) {
        ...
        console.error(`OpenAI error ${status}`);
        process.exit(1);
      }
